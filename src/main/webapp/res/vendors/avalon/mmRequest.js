...
//  数据交互模块 by 司徒正美
//==========================================
define("mmRequest", ["avalon","base64","cookie", "mmPromise"], function(avalon,base64) { 
...
//        为处理后端跳转dcas验证造成的跨域问题 by 赵正兵 2015-03-23
        if (status === 420) {
        	...
        	var encodedHref = base64.encodeUrlsafe(location.href);
        	// 添加backurl
        	validateUrl += (rquery.test(validateUrl) ? "&" : "?") + "frontBackurl=" + encodeURIComponent(encodedHref);
        	
        	var languages = avalon.cookie.get('CARGOPM_LANG_COOKIE_NAME');
        	if(!languages){
        		languages = (navigator.language || navigator.userLanguage || navigator.browserLanguage || navigator.systemLanguage).toLowerCase();	
        	}
